...
import cv2
import json
from detector import detector, tracker, draw_boxes_on_image
import sqlite3
import argparse
import torch
import uuid
        ...
        while True:
            ...
            for box in slice_boxes:
                ...
                for i, det in enumerate(proc_pred):
                    proc_pred[i] = proc_pred[i].detach().clone()
                    proc_pred[i][0] += box[0]
            ...
            if save:
                try:
                    cv2.imwrite(dest+'/'+im_name+'_with_labels.jpg', draw_boxes_on_image(img0, preds[:, :4].tolist(), [model.names[int(i)] for i in preds[:, 5].tolist()], preds[:, 5].tolist(), preds[:, 4].tolist()))
                except:
